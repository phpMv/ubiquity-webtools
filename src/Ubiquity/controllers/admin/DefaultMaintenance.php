...
abstract class DefaultMaintenance extends ControllerBase {
	...
	protected function createHtmlDimmer() {
		...
		$dimmer->asIcon($this->activeMaintenance->getIcon(), $this->activeMaintenance->getTitle(), $this->activeMaintenance->getMessage())
			->asPage();
	...
	protected function createTimer(HtmlDimmer $dimmer, $duration) {
		...
		$seg->setContent([
			$this->getLabel('day'),
			$this->getLabel('hour'),
			$this->getLabel('minute'),
			$this->getLabel('second')
		]);
		$seg->addClass('fluid');
	...
	public function comingSoon() {
		...
		if (URequest::isPost()) {
			...
			$frm->setSubmitParams($ctrl . '/comingSoon', '#frm-register', [
				'hasLoader' => 'internal'
			]);
